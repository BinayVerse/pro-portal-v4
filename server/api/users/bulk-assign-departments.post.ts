...
export default defineEventHandler(async (event: H3Event) => {
  ...
  try {
    // 🔑 PERMISSION CHECK 1: Validate Department Admin can only assign to their own departments
    if (callerRole === 3) {
    ...
    // Department Admin cannot bulk assign other DAs or CAs
    if (callerRole === 3) {
    ...
    if (callerRole === 3) {
      ...
      for (const targetUserId of userIds) {
        ...
        if (!hasOverlap && !isUnassigned) {
          setResponseStatus(event, 403)
          throw new CustomError(
            `You can only assign users that are in your departments or unassigned`,
            403
          )
        }
      }
    ...
    // 🔑 BULK INSERT: Add all user-department combinations
    ...
    return {
      ...
      message: `Successfully assigned ${userIds.length} user(s) to ${departments.length} department(s)`,
    }
  } catch (error: any) {
